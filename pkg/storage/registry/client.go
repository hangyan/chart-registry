...
func (c *Client) LoadChart(ref *Reference) (*ChartObject, error) {
	r, err := c.cache.FetchReference(ref)
	if err != nil {
		return nil, err
	}
...
func (c *Client) ListCharts() ([]*ChartObject, error) {
	rr, err := c.cache.ListReferences()
	if err != nil {
		return nil, err
	}
	var result []*ChartObject
	for _, item := range rr {
		result = append(result, item.ToChartObject())
	}
	return result, nil
